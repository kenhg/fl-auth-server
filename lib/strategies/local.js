  ...
  _createClass(RegisterStrategy, [{
    ...
    value: function authenticate(req) {
      ...
      this.verify(req, email, password, function (err, user, info) {
        ...
        (0, _lib.findOrCreateAccessToken)({ user_id: user.id }, { expires: true }, function (err, access_token, refresh_token, info) {
          if (err) return _this.error(err);
          ...
          _this.success(_lodash2['default'].pick.apply(_lodash2['default'], [user.toJSON(), 'id', 'email'].concat(_toConsumableArray(_this.extra_register_params))), { access_token: access_token });
        });
      });
    }
  }]);
