...
var RegisterStrategy = (function (_Strategy) {
  ...
  RegisterStrategy.prototype.authenticate = function authenticate(req) {
    ...
    this.verify(req, email, password, function (err, user, info) {
      ...
      _lib.findOrCreateAccessToken({ user_id: user.id }, { expires: true }, function (err, token, refresh_token, info) {
        if (err) return _this.error(err);

        req.session.access_token = { token: token, expires_at: info.expires_at };
        req.session.save(function (err) {
          if (err) console.log('Error saving session', err);
        });
        _this.success(_lodash2['default'].omit(user.toJSON(), 'password'), { access_token: token });
      });
    });
  };
