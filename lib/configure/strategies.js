...
function configureStrategies() {
  ...
  if (options.facebook && options.facebook.app_id && options.facebook.app_secret) {
    ...
    }, function (token, refresh_token, profile, callback) {
  ...
  if (options.linkedin && options.linkedin.app_id && options.linkedin.app_secret) {
    _passport2['default'].use(new _passportLinkedinOauth2.Strategy({
      ...
      User.findOrCreate({ email: email }, function (err, user) {
        ...
        user.save({ linkedin_id: profile.id, linkedin_access_token: token }, function (err) {
          if (err) return callback(err);

          options.linkedin.onUserCreated(user, profile, function (err) {
            if (err) return callback(err);
            callback(null, user);
          });
        });
