...
function configureRoutes() {
  ...
  app.post(options.paths.reset, function (req, res) {
    var _req$body = req.body;
    var reset_token = _req$body.reset_token;
    ...
    User.findOne({ reset_token: reset_token }, function (err, user) {
      ...
      console.log('comparing token age, now:', _moment2['default'].utc());
      ...
      console.log('diff:', _moment2['default'].diff(_moment2['default'].utc(), user.get('reset_token_created_at')));
      console.log('js -:', new Date() - user.get('reset_token_created_at'));

      if (_moment2['default'].diff(_moment2['default'].utc(), (0, _moment2['default'])(user.get('reset_token_created_at'))) > options.reset_token_expires_ms) return res.status(402).json({ error: 'This token has expired' });
