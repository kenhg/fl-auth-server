...
function configureRoutes() {
  ...
  app.post(options.paths.resetRequest, function (req, res) {
    ...
    User.findOne({ email: email }, function (err, user) {
      ...
      user.save({ resetToken: _lib.createToken(), resetToken_createdDate: _moment2['default'].utc().toDate() }, function (err) {
        if (err) return _lib.sendError(res, err);
  ...
  app.post(options.paths.confirm, function (req, res) {
    ...
    User.findOne({ email: email, emailConfirmationToken: token }, function (err, user) {
      ...
      user.save({ emailConfirmationToken: null, emailConfirmed_at: _moment2['default'].utc().toDate() }, function (err) {
        if (err) return _lib.sendError(res, err);
        res.status(200).send({});
      });
    });
  ...
  if (options.linkedin) {
    app.get(options.linkedin.paths.redirect, _passport2['default'].authenticate('linkedin'));
    app.get(options.linkedin.paths.callback, _passport2['default'].authenticate('linkedin', { successRedirect: '/', failureRedirect: options.paths.login }));
  }
}
