...
function configureSerializing() {
  ...
  _passport2['default'].serializeUser(options.serializing.serializeUser || function (user, callback) {
    if (!user) return callback(new Error('[fl-auth] User missing'));
    callback(null, user.id);
  });

  _passport2['default'].deserializeUser(options.serializing.deserializeUser || function (id, callback) {
