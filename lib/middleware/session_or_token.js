...
function sessionOrToken(req, res, next) {
  ...
  _passport2['default'].authenticate('bearer', { session: false }, function (err, user) {
    ...
    if (!user) {
      if (req.method === 'get') return res.redirect(302, '/login?redirect_to=' + req.url);
...
//      res.redirect(302,  `/login?redirect_to=${req.url}`)

//   else
//     # check access token expiry
//     (req.logout(); return res.redirect(302,  `/login?redirect_to=${req.url}`)) unless access_token = req.session?.access_token

//     expires_at = access_token.expires_at
//     if expires_at? and moment().isAfter(expires_at)
//       vidigami = passport._strategy('vidigami')
//       (req.logout(); return res.redirect(302,  `/login?redirect_to=${req.url}`)) unless vidigami
//       vidigami.refreshToken access_token.refresh_token, (err, access_token) ->
//         (req.logout(); return res.redirect(302,  `/login?redirect_to=${req.url}`)) if err or not access_token?.id
